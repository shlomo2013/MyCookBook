...
public class DropDownListAdapter extends BaseAdapter {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			holder.tv = (TextView) convertView.findViewById(R.id.tvSelectOption);
		...
		if(PersonalFragment.checkSelected[position])
	...
	private void setText(int position1){
		if (!PersonalFragment.checkSelected[position1]) {
			PersonalFragment.checkSelected[position1] = true;
			selectedCount++;
		} else {
            PersonalFragment.checkSelected[position1] = false;
			selectedCount--;
		}
		...
		if (selectedCount == 0) {
			...
			for (int i = 0; i < PersonalFragment.checkSelected.length; i++) {
				if (PersonalFragment.checkSelected[i] == true) {
